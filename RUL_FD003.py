...
# -*- coding: utf-8 -*-
"""
Created on Wed Jan 12 17:30:09 2022
...
FD003_df = pd.merge(data_act, max_cycles_df, how="inner", on="ID")
FD003_df["RUL"] = FD003_df["MaxCycleID"] - FD003_df["Cycle"]
grouped_k_m = FD003_df.groupby(['op_set_net']).mean()
print(grouped_k_m)
print(grouped_k_m.loc[0,'SensorMeasure1'])
grouped_k_std = FD003_df.groupby(['op_set_net']).std()
print(grouped_k_std)
...
sensors = ['SensorMeasure2', 'SensorMeasure3', 'SensorMeasure4',
           ...
           'SensorMeasure21']
def get_norm_df(main_df = FD003_df,mean_df = grouped_k_m, std_df = grouped_k_std, columns = sensors, num_index = FD003_df.shape[0]):
...
FD003_norm_df = get_norm_df()
FD003_norm_df[['ID','Cycle','op_set_net','RUL']] = FD003_df.loc[:,['ID','Cycle','op_set_net','RUL']]
enc_opset = OneHotEncoder()
opset_enc = enc_opset.fit_transform(FD003_norm_df['op_set_net'].values.reshape(-1,1)).toarray()
print(opset_enc.shape)
x_num_ar = FD003_norm_df[sensors].values
ID_ar = FD003_norm_df['ID'].values.reshape(-1,1)
y_ar = FD003_norm_df['RUL'].values.reshape(-1,1)
final_tr_df = pd.DataFrame(np.concatenate([ID_ar,opset_enc,x_num_ar,y_ar],axis = 1), 
                           columns = ['ID','enc_set1', 'enc_set2','SensorMeasure2', 'SensorMeasure3', 'SensorMeasure4',
                                      'SensorMeasure6', 'SensorMeasure7', 'SensorMeasure8', 
                                      'SensorMeasure9', 'SensorMeasure10', 'SensorMeasure11', 
                                      'SensorMeasure12', 'SensorMeasure13', 'SensorMeasure14', 
                                      'SensorMeasure15', 'SensorMeasure17', 'SensorMeasure20', 
...
for g in group:
    ...
    in_ar = df[['enc_set1', 'enc_set2','SensorMeasure2', 'SensorMeasure3', 'SensorMeasure4',
                'SensorMeasure6', 'SensorMeasure7', 'SensorMeasure8', 
                'SensorMeasure9', 'SensorMeasure10', 'SensorMeasure11', 
                'SensorMeasure12', 'SensorMeasure13', 'SensorMeasure14', 
                'SensorMeasure15', 'SensorMeasure17', 'SensorMeasure20', 
...
test_data_FD003 = pd.read_csv('test_FD003.txt', sep = ' ', header = None)
test_data_FD003 = test_data_FD003[[f for f in range(0, 26)]]
test_data_FD003.columns = ["ID", "Cycle", "OpSet1", "OpSet2", "OpSet3", 
...
max_cycles_df_test = test_data_FD003.groupby(["ID"], sort=False)["Cycle"].max().reset_index().rename(columns={"Cycle" : "MaxCycleID"})
y_test_rul_FD003 = pd.read_csv('RUL_FD003.txt', header = None)
y_test_rul_FD003.columns = ['Eng_rul']
y_test_rul_FD003['ID'] = max_cycles_df_test['ID']

max_cycles_df_test['MaxCycleIDAct'] = y_test_rul_FD003['Eng_rul']+ max_cycles_df_test['MaxCycleID']
test_df_FD003 = pd.merge(test_data_FD003, max_cycles_df_test, how="inner", on="ID")
test_df_FD003["RUL"] = test_df_FD003["MaxCycleIDAct"] - test_df_FD003["Cycle"]
data_op_set_te = test_df_FD003[['OpSet1','OpSet2','OpSet3']]
...
test_df_FD003['op_set_net'] = label_te
test_df_FD003_norm = get_norm_df(main_df=test_df_FD003, num_index=test_df_FD003.shape[0])
test_df_FD003_norm[['ID','Cycle','op_set_net','RUL']] = test_df_FD003.loc[:,['ID','Cycle','op_set_net','RUL']]
opset_enc_te = enc_opset.transform(test_df_FD003_norm['op_set_net'].values.reshape(-1,1)).toarray()
print(opset_enc.shape)
x_num_ar_te = test_df_FD003_norm[sensors].values
ID_ar_te = test_df_FD003_norm['ID'].values.reshape(-1,1)
y_ar_te = test_df_FD003_norm['RUL'].values.reshape(-1,1)
final_te_df = pd.DataFrame(np.concatenate([ID_ar_te,opset_enc_te,x_num_ar_te,y_ar_te],axis = 1), 
                           columns = ['ID','enc_set1', 'enc_set2','SensorMeasure2', 
                                      'SensorMeasure3', 'SensorMeasure4',
                                      'SensorMeasure6', 'SensorMeasure7', 'SensorMeasure8', 
                                      'SensorMeasure9', 'SensorMeasure10', 'SensorMeasure11', 
                                      'SensorMeasure12', 'SensorMeasure13', 'SensorMeasure14', 
                                      'SensorMeasure15', 'SensorMeasure17', 'SensorMeasure20', 
...
x_tr_cnn = np.asarray(x_tr_cnn).astype(np.float32)
x_tr_f_cnn = x_tr_f[:,:,:,np.newaxis]
x_tr_f_cnn = np.asarray(x_tr_f_cnn).astype(np.float32)
x_tr_w_cnn = x_tr_w[:,:,:,np.newaxis]
x_tr_w_cnn = np.asarray(x_tr_w_cnn).astype(np.float32)
print(x_tr_cnn.shape)
print(x_tr_f_cnn.shape)
print(x_tr_w_cnn.shape)
x_te_cnn = x_te[:,:,:,np.newaxis]
x_te_cnn = np.asarray(x_te_cnn).astype(np.float32)
x_te_f_cnn = x_te_f[:,:,:,np.newaxis]
x_te_f_cnn = np.asarray(x_te_f_cnn).astype(np.float32)
x_te_w_cnn = x_te_w[:,:,:,np.newaxis]
x_te_w_cnn = np.asarray(x_te_w_cnn).astype(np.float32)
...
res_1.to_csv('result_rul_0601_tr_FD003.csv')
...
res_1 = pd.DataFrame(np.concatenate([rul_pred_naive_te,rul_pred_fourier_te,rul_pred_wavelet_te,rul_pred_naive_bilstm_te,rul_pred_fourier_bilstm_te,rul_pred_wavelet_bilstm_te,rul_pred_naive_cnn_te,rul_pred_fourier_cnn_te,rul_pred_wavelet_cnn_te,y_te], axis = 1), 
                     columns=['predicted_RUL_naive','predicted_RUL_fourier','predicted_RUL_wavelet','predicted_RUL_Naive_BILSTM','predicted_RUL_fourier_BILSTM','predicted_RUL_wavelet_BILSTM','predicted_RUL_Naive_CNN','predicted_RUL_fourier_CNN','predicted_RUL_wavelet_CNN','Actual_RUL'])
res_1.to_csv('result_rul_0601_te_FD003.csv')
