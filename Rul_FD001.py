...
grouped_k_m = FD001_df.groupby(['op_set_net']).mean()
print(grouped_k_m)
print(grouped_k_m.loc[0,'SensorMeasure1'])
grouped_k_std = FD001_df.groupby(['op_set_net']).std()
print(grouped_k_std)
...
sensors = ['SensorMeasure2','SensorMeasure3', 'SensorMeasure4', 
           ...
           'SensorMeasure14', 'SensorMeasure15','SensorMeasure17', 
           'SensorMeasure20', 'SensorMeasure21']

def get_norm_df(main_df = FD001_df,mean_df = grouped_k_m, std_df = grouped_k_std, columns = sensors, num_index = FD001_df.shape[0]):
...
FD001_norm_df = get_norm_df()
FD001_norm_df[['ID','Cycle','op_set_net','RUL']] = FD001_df.loc[:,['ID','Cycle','op_set_net','RUL']]
enc_opset = OneHotEncoder()
opset_enc = enc_opset.fit_transform(FD001_norm_df['op_set_net'].values.reshape(-1,1)).toarray()
print(opset_enc.shape)

x_num_ar = FD001_norm_df[sensors].values
ID_ar = FD001_norm_df['ID'].values.reshape(-1,1)
y_ar = FD001_norm_df['RUL'].values.reshape(-1,1)
...
for g in group:
    ...
    in_ar = df[['enc_set1', 'enc_set2','SensorMeasure2',
                ...
                'SensorMeasure17', 'SensorMeasure20', 'SensorMeasure21']].values
...
test_data_FD001.columns = ["ID", "Cycle", "OpSet1", "OpSet2", "OpSet3", 
...
print(test_df_FD001.head())

test_df_FD001["RUL"] = test_df_FD001["MaxCycleIDAct"] - test_df_FD001["Cycle"]
print(test_df_FD001.head())

data_op_set_te = test_df_FD001[['OpSet1','OpSet2','OpSet3']]
...
test_df_FD001['op_set_net'] = label_te
test_df_FD001_norm = get_norm_df(main_df=test_df_FD001, num_index=test_df_FD001.shape[0])
test_df_FD001_norm[['ID','Cycle','op_set_net','RUL']] = test_df_FD001.loc[:,['ID','Cycle','op_set_net','RUL']]
opset_enc_te = enc_opset.transform(test_df_FD001_norm['op_set_net'].values.reshape(-1,1)).toarray()
print(opset_enc.shape)

x_num_ar_te = test_df_FD001_norm[sensors].values
ID_ar_te = test_df_FD001_norm['ID'].values.reshape(-1,1)
y_ar_te = test_df_FD001_norm['RUL'].values.reshape(-1,1)
...
res_1.to_csv('result_rul_0601_tr_FD001.csv')
...
res_1 = pd.DataFrame(np.concatenate([rul_pred_naive_te,rul_pred_fourier_te,rul_pred_wavelet_te,rul_pred_naive_bilstm_te,rul_pred_fourier_bilstm_te,rul_pred_wavelet_bilstm_te,rul_pred_naive_cnn_te,rul_pred_fourier_cnn_te,rul_pred_wavelet_cnn_te,y_te], axis = 1), 
                     columns=['predicted_RUL_naive','predicted_RUL_fourier','predicted_RUL_wavelet','predicted_RUL_Naive_BILSTM','predicted_RUL_fourier_BILSTM','predicted_RUL_wavelet_BILSTM','predicted_RUL_Naive_CNN','predicted_RUL_fourier_CNN','predicted_RUL_wavelet_CNN','Actual_RUL'])
res_1.to_csv('result_rul_0601_te_FD001.csv')
