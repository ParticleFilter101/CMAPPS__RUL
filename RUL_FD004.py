...
# -*- coding: utf-8 -*-
"""
Created on Wed Jan 12 18:21:01 2022
...
data = pd.read_csv('train_FD004.txt', sep = ' ', header = None)
...
FD004_df = pd.merge(data_act, max_cycles_df, how="inner", on="ID")
FD004_df["RUL"] = FD004_df["MaxCycleID"] - FD004_df["Cycle"]
grouped_k_m = FD004_df.groupby(['op_set_net']).mean()
print(grouped_k_m)
print(grouped_k_m.loc[0,'SensorMeasure1'])
grouped_k_std = FD004_df.groupby(['op_set_net']).std()
print(grouped_k_std)
...
def get_norm_df(main_df = FD004_df,mean_df = grouped_k_m, std_df = grouped_k_std, columns = sensors, num_index = FD004_df.shape[0]):
    ...
    return norm_df
FD004_norm_df = get_norm_df()
FD004_norm_df[['ID','Cycle','op_set_net','RUL']] = FD004_df.loc[:,['ID','Cycle','op_set_net','RUL']]
enc_opset = OneHotEncoder()
opset_enc = enc_opset.fit_transform(FD004_norm_df['op_set_net'].values.reshape(-1,1)).toarray()
print(opset_enc.shape)
x_num_ar = FD004_norm_df[sensors].values
ID_ar = FD004_norm_df['ID'].values.reshape(-1,1)
y_ar = FD004_norm_df['RUL'].values.reshape(-1,1)
final_tr_df = pd.DataFrame(np.concatenate([ID_ar,opset_enc,x_num_ar,y_ar],axis = 1), 
                           columns = ['ID','enc_set1', 'enc_set2','enc_set3','enc_set4',
                                      'enc_set5','enc_set6','SensorMeasure2', 'SensorMeasure3', 'SensorMeasure4',
                                      'SensorMeasure6', 'SensorMeasure7', 'SensorMeasure8', 
                                      'SensorMeasure9','SensorMeasure10', 'SensorMeasure11', 
                                      'SensorMeasure12','SensorMeasure13', 'SensorMeasure14', 
                                      'SensorMeasure15','SensorMeasure17','SensorMeasure20', 
...
test_data_FD004 = pd.read_csv('test_FD004.txt', sep = ' ', header = None)
test_data_FD004 = test_data_FD004[[f for f in range(0, 26)]]
test_data_FD004.columns = ["ID", "Cycle", "OpSet1", "OpSet2", "OpSet3", 
...
max_cycles_df_test = test_data_FD004.groupby(["ID"], sort=False)["Cycle"].max().reset_index().rename(columns={"Cycle" : "MaxCycleID"})
y_test_rul_FD004 = pd.read_csv('RUL_FD004.txt', header = None)
y_test_rul_FD004.columns = ['Eng_rul']
y_test_rul_FD004['ID'] = max_cycles_df_test['ID']
max_cycles_df_test['MaxCycleIDAct'] = y_test_rul_FD004['Eng_rul']+ max_cycles_df_test['MaxCycleID']
test_df_FD004 = pd.merge(test_data_FD004, max_cycles_df_test, how="inner", on="ID")
test_df_FD004["RUL"] = test_df_FD004["MaxCycleIDAct"] - test_df_FD004["Cycle"]
data_op_set_te = test_df_FD004[['OpSet1','OpSet2','OpSet3']]
...
test_df_FD004['op_set_net'] = label_te
test_df_FD004_norm = get_norm_df(main_df=test_df_FD004, num_index=test_df_FD004.shape[0])
test_df_FD004_norm[['ID','Cycle','op_set_net','RUL']] = test_df_FD004.loc[:,['ID','Cycle','op_set_net','RUL']]
opset_enc_te = enc_opset.transform(test_df_FD004_norm['op_set_net'].values.reshape(-1,1)).toarray()
print(opset_enc.shape)
x_num_ar_te = test_df_FD004_norm[sensors].values
ID_ar_te = test_df_FD004_norm['ID'].values.reshape(-1,1)
y_ar_te = test_df_FD004_norm['RUL'].values.reshape(-1,1)
...
res_1.to_csv('result_rul_0601_tr_FD004.csv')
...
res_1 = pd.DataFrame(np.concatenate([rul_pred_naive_te,rul_pred_fourier_te,rul_pred_wavelet_te,rul_pred_naive_bilstm_te,rul_pred_fourier_bilstm_te,rul_pred_wavelet_bilstm_te,rul_pred_naive_cnn_te,rul_pred_fourier_cnn_te,rul_pred_wavelet_cnn_te,y_te], axis = 1), 
                     columns=['predicted_RUL_naive','predicted_RUL_fourier','predicted_RUL_wavelet','predicted_RUL_Naive_BILSTM','predicted_RUL_fourier_BILSTM','predicted_RUL_wavelet_BILSTM','predicted_RUL_Naive_CNN','predicted_RUL_fourier_CNN','predicted_RUL_wavelet_CNN','Actual_RUL'])
res_1.to_csv('result_rul_0601_te_FD004.csv')